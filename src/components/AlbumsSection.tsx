...
type AlbumsSectionProps = {
  ...
  openFlipPreview: (albumId: string) => void;
};
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        {visibleAlbums.map((album) => {
          const albumImageCount =
            uploadsByAlbum[album.id]?.filter((item) => item.type === "image").length ?? 0;
          const canOpenFlipPreview = albumImageCount >= 2;
                  ...
                  >
                    ...
                    >
                      {coverItem ? (
                        ...
                        <Image
                          src={safeAlbumSrc}
                          alt={album.alt || `${album.title} cover`}
                          fill
              ...
              <div className="flex items-center gap-2">
                  {canOpenFlipPreview ? (
                    <button
                      ...
                      onClick={() => openFlipPreview(album.id)}
                    >
                      flip preview
                    </button>
                  ) : null}
